document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayAttendanceReport(students, attendanceData, date, grade) {
        ...
        tbody.innerHTML = reportData.map(item => {
            ...
            ${statusIcon} ${item.status.toUpperCase()}
                ...
                <td>
                    ...
                    data-student-name="${item.student.firstName} ${item.student.lastName}"
        ...
        reportCard.style.display = 'block';
            // Attach click handlers to Edit buttons (safer than inline onclick)
        ...
        editButtons.forEach(btn => {
            ...
            // call your modal opener with same contract as before
            openEditModal(studentId, studentName, status, grade);
         });
        });
    }
...
window.openEditModal = function(studentId, fullName, currentStatus, grade) {
    editingStudentId = studentId;
    editingGrade = grade;
    editingSection = getSectionName(grade);
    editingDate = document.getElementById('attendanceDate').value;
...
window.saveAttendanceEdit = function() {
    ...
    const key = `attendance_${editingDate}_${editingGrade}_${editingSection}`;
    let attendanceData = JSON.parse(localStorage.getItem(key) || '[]');

    // find student
    const rec = attendanceData.find(a => a.studentId === editingStudentId);
    if (rec) {
    ...
    closeEditModal();
    loadAttendanceReport(); // refresh UI
};
